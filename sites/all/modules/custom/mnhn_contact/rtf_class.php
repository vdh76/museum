...
//-------------------------------------------------------------------------------------------------
	function RTF($inc_file) {
		...
		$this->tbl_def_border_width=$tbl_border_width;
...
//-------------------------------------------------------------------------------------------------
		...
		$string=preg_replace("/<tpagenum>/msi",$fig_l.$slash."field".$fig_l.$slash.$star.$slash."fldinst  ".$fig_l." NUMPAGES ".$fig_r.$fig_r.$fig_r,$string);
		$string=preg_replace("/\n/msi", "", $string);
		...
		$string=preg_replace("/<DEL>/msi", $slash."strike ", $string);
		$string=preg_replace("/<\/DEL>/msi", $slash."strike0 ", $string);
...
//-------------------------------------------------------------------------------------------------
	function parce_table_new($table) {
		$rowkeep = false;
		$result = "";
			unset($border_width);unset($tbl_border);unset($tbl_width);unset($tbl_height);unset($tbl_align);unset($tbl_valign);unset($tbl_bgcolor);unset($bord_color);
			$all_data_head = array();
			$all_data_body = array();
			$all_data_wdth = array();
			$tmp = split(">", $table);
			...
			if (isset($border)) { $tbl_border = $border; unset($border); }
			else {$tbl_border = $this->tbl_def_border;}
			if (isset($border_width)) { $tbl_border_width = $this->twips($border_width); unset($border_width); }
			else {$tbl_border_width = $this->twips($this->tbl_def_border_width);}
			if (isset($width)) { $tbl_width = $width; unset($width); }
			else {$tbl_width = $this->tbl_def_width;}
			if (isset($align)) { $tbl_align = $align; unset($align); }
			else {$tbl_align = $this->tbl_def_align;}
			if (isset($valign)) { $tbl_valign = $valign; unset($valign); }
			else {$tbl_valign = $this->tbl_def_valign;}
			if (isset($bgcolor)) { $tbl_bgcolor = $bgcolor; unset($bgcolor); }
			...
			if (preg_match_all("/(<tr)(.*?)(<\/tr>)/msi", $other, $trs))
			{
				...
				for ($r=0;$r<sizeof($trs);$r++) {
					...
					$perms = strtolower($perms);
					parse_str($perms);
					if (isset($cellpadding)) { $tr_cellpadding = $this->twips($cellpadding); unset($cellpadding); }
					else {$tr_cellpadding = $tbl_cellpadding;}
					if (isset($border)) { $tr_border = $border; unset($border); }
					else {$tr_border = $tbl_border;}
					if (isset($width)) { $tr_width = $width; unset($width); }
					else {$tr_width = $tbl_width;}
					if (isset($height)) { $tr_height = $height; unset($height); }
					else {$tr_height = 0;}
					if (isset($align)) { $tr_align = $align; unset($align); }
					else {$tr_align = $this->row_def_align;}
					if (isset($valign)) { $tr_valign = $valign; unset($valign); }
					else {$tr_valign = $tbl_valign;}
					if (isset($bgcolor)) { $tr_bgcolor = $bgcolor; unset($bgcolor); }
					...
					if (preg_match_all("/(<td)(.*?)(<\/td>)/msi", $other2, $tds)) {
						...
						for ($d=0;$d<sizeof($tds);$d++) {
							...
							$perms = strtolower($perms);
							parse_str($perms);
							if (isset($colspan)) { $td_colspan = $colspan; unset($colspan); }
							else {$td_colspan =1;}
							if (isset($rowspan)) { $td_rowspan = $rowspan; unset($rowspan); }
							else {$td_rowspan =1;}
							if (isset($border)) { $td_border = $border; unset($border); }
							else {$td_border = $tr_border;}
							if (isset($width)) { $td_width = $width; unset($width); }
							else {$td_width = "no";}
							if (isset($align)) { $td_align = $align; unset($align); }
							else {$td_align = $tr_align;}
							if (isset($valign)) { $td_valign = $valign; unset($valign); }
							else {$td_valign = $tr_valign;}
							if (isset($bgcolor)) { $td_bgcolor = $bgcolor; unset($bgcolor); }
...
////////////////////////// - CELLS
							...
							unset($color);
							if ($td_border==1) {
								$td_brd_f = "\\clbrdrt\\brdrs\\brdrw".$tbl_border_width."".$brd_color."\\clbrdrl\\brdrs\\brdrw".$tbl_border_width."".$brd_color."\\clbrdrb\\brdrs\\brdrw".$tbl_border_width."".$brd_color."\\clbrdrr\\brdrs\\brdrw".$tbl_border_width."".$brd_color;
							}
							else {
								$td_brd_f = "";
								if (preg_match("/t/",$td_border)) { $td_brd_f .= "\\clbrdrt\\brdrs\\brdrw".$tbl_border_width."".$brd_color; }
								if (preg_match("/b/",$td_border)) { $td_brd_f .= "\\clbrdrb\\brdrs\\brdrw".$tbl_border_width."".$brd_color; }
								if (preg_match("/r/",$td_border)) { $td_brd_f .= "\\clbrdrr\\brdrs\\brdrw".$tbl_border_width."".$brd_color; }
								if (preg_match("/l/",$td_border)) { $td_brd_f .= "\\clbrdrl\\brdrs\\brdrw".$tbl_border_width."".$brd_color; }
							}
							if (ereg("%",$td_width)) {
