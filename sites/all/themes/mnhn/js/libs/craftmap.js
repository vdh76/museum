    ...
    $.fn.craftmap = function(options) {
		var D = {
			...
			tip: {
				...
				popin: true,
				popin_name: 'popin',
				onClick: function(tip, popin){},
				onClose: function(tip, popin){}
			},
			controls: {
				init: true,
				name: 'controls',
								...
								if (marker.popup){
									...
									t.after(
										$('<div />').addClass(marker.popup_name+' '+id).css(css).html(html).append(
											$('<a />').addClass('close').attr('href','javascript:;')
										)
									);
					...
					tip: {
						...
						set: function(){
							TIP.each(function(){
								...
								t.css(css);
							}).wrapInner($('<div />').addClass(S.tip.name+'Content').hide());
						},
						open: function(){
							TIP.live('click', function(){
								var t = $(this), id = t.attr('id'), tip = S.tip, w = t.width(), h = t.height(),
									position = t.position(), x = position.left, y = position.top, id = t.attr('id'),
									html = t.find('.'+tip.name+'Content').html();
								...
								if (tip.popin){
									$('.'+tip.popin_name).remove();
									var css = {
										position:'absolute', 
										zIndex:'4'
									}
									t.after(
										$('<div />').addClass(tip.popin_name+' '+id).css(css).html(html).append(
											$('<a />').addClass('close').attr('href','javascript:;')
										)
									);
									var POPIN = t.next('.'+tip.popin_name), 
										pw = POPIN.innerWidth(), 
										ph = POPIN.innerHeight(),
										x0 = 0, y0 = 0;
								...
								P.controls.active.set(id);
								tip.onClick.call(this, t, POPIN); 
								return false;
							});
						},
						close: function(){
							C.find('.close').live('click', function(){
								var t = $(this), popin = t.parents('.'+S.tip.popin_name), tip = popin.prev('.'+S.tip.name);
								popin.remove();
								P.controls.active.remove();
								S.tip.onClose.call(this, tip, popin);
								return false;
							});
						}
					},
