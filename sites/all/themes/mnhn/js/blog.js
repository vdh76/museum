jQuery(document).ready(function() {
	...
	jQuery("#comment .link").click(function(){
		...
		if(jQuery("#comment #commentaire").val().length<30){
			error=1;
		...
		if(jQuery("#recaptcha_response_field").val()==0){
			error=1;
			message+="Les caractères de vérification sont inexacts.\n";
		}
					  ...
					  success: function(data){
						  if(data!="ok"){
							...
							jQuery.ajax({
								  ...
								  type: 'POST',
								  data: {nom: jQuery("#comment #nom").val(),commentaire: jQuery("#comment #commentaire").val()},
								  dataType: "html",
								  success: function(data){
									  	  Recaptcha.reload();
									  	  //jQuery(".comment_wrap").prepend(data);
